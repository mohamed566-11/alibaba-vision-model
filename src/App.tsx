...
import { VirtualTryOnArea } from './components/VirtualTryOnArea';
import type {
  GarmentType, BackgroundType, QualityType,
  GenerationHistoryItem, GarmentInventory,
  SavedGarmentItem, ExtractedItemResult,
  ExtractionStage, ExtractionResponse, ActiveTab
} from './types';
import { Sparkles, Scissors, AlertCircle, X, Shirt } from 'lucide-react';

export default function App() {
  const [activeTab, setActiveTab]       = useState<ActiveTab>('extractor');
  ...
  return (
    <div className="min-h-screen bg-[#FBFBFA] font-sans selection:bg-zinc-200">
      ...
      {/* Navigation Header */}
      <header className="w-full bg-white border-b border-zinc-100 sticky top-0 z-50 shadow-2xs">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-zinc-900 rounded-xl flex items-center justify-center shadow-xs">
              <Scissors className="w-4.5 h-4.5 text-white" />
            </div>
            <div>
              <div className="flex items-center gap-1.5">
                <span className="font-bold text-zinc-900 tracking-tight">Alta</span>
                <span className="text-zinc-300">/</span>
                <span className="text-xs font-semibold text-zinc-500">AI Fashion Suite</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 p-1 bg-zinc-100/80 rounded-xl border border-zinc-200/60">
            ...
            >
              ...
              <span className="px-1.5 py-0.5 text-[9px] font-extrabold bg-emerald-600 text-white rounded-full uppercase tracking-wider">
                NEW
              </span>
            </button>
          </div>

          <span className="hidden md:flex items-center gap-2 text-xs font-medium px-2.5 py-1 bg-zinc-100 text-zinc-600 rounded-md">
            <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
            qwen-image-2.0-pro · Try-On & Closet
          </span>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
        {activeTab === 'extractor' ? (
          ...
          <VirtualTryOnArea 
            savedGarments={savedGarments}
            onSwitchToExtractor={() => setActiveTab('extractor')}
          />
        )}
      </main>
